...
private static byte[] Color32ArrayToByteArray(Color32[] colors)
{
    ...
    return bytes;
		...
		public IEnumerator newAddImageByTexture ( Texture2D image, int X, int Y ) {
			
			yield return new WaitForSeconds(1);
			Debug.Log(image.GetRawTextureData().Length);
			//Byte[] colorByte = Color32ArrayToByteArray(image.GetRawTextureData<Color32>().ToArray()); 
			addImage(image.GetRawTextureData(),X,Y,image.height,image.width);
			Debug.Log ("HEIGTH :"+image.height+" WIDTH :"+image.width );
		}
/**
			public IEnumerator newAddImage ( string imageName, int X, int Y ) {
			//WWW www = new WWW(imageName);
			UnityEngine.Networking.UnityWebRequest www = UnityEngine.Networking.UnityWebRequestTexture.GetTexture(imageName);
			...
			Debug.Log ( "Adding Image Length "+www.downloadHandler.data.Length+" HEIGTH :"+((UnityEngine.Networking.DownloadHandlerTexture)www.downloadHandler).texture.height+" WIDTH :"+((UnityEngine.Networking.DownloadHandlerTexture)www.downloadHandler).texture.width );
		}*/
